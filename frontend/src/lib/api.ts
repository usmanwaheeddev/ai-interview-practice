...
/**
 * Server-Sent Events over a POST body — the AI review/hint endpoints need a
 * request body (candidate code, cursor position), which the native
 * EventSource API can't send (GET-only), so this parses the same "data: "/
 * "event: " framing by hand over a plain fetch + ReadableStream. Only used
 * by the coding-challenge AI review/hint features so far.
 */
export async function streamSse(path: string, body: unknown, handlers: SseHandlers): Promise<void> {
  ...
  while (true) {
    ...
    while (boundary !== -1) {
      ...
      if (eventType === "error") {
        throw new ApiError(0, "stream_error", data);
      }
      if (eventType === "done") {
